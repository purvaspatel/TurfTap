...
function SportsCityPageContent() {
  ...
  const { sports } = useParams(); // Get dynamic sport & city from URL
  ...
  useEffect(() => {
    async function fetchGrounds() {
      const res = await fetch(`/api/grounds?sport=${sports}&sort=upvotes&page=${page}`);
      if (res.ok) {
  ...
  return (
    <>
      {grounds.length === 0 ? (
        <NoResults message={`No grounds found for ${sports}. Try searching for another location - Contribute if you know nearby grounds/turfs!`} />
      ) : (
        <div className="container mx-auto max-w-6xl p-6">
          ...
          <div className="mt-8 flex justify-between">
            <Button
              variant="outline"
              disabled={page <= 1}
              onClick={() => router.push(`/sports/${sports}/?page=${page - 1}`)}
            >
              ← Previous
            </Button>
